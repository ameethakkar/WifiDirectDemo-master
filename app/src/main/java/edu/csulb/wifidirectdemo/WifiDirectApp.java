...
public class WifiDirectApp extends Application {
   ...
   public boolean mIsServer = false;
    ...
    public boolean isP2pEnabled() {
        String state = SysPreferences.getStringFromPref(this, SysPreferences.PREF_NAME, SysPreferences.P2P_ENABLED);
    ...
    public String shiftInsertMessage(MessageDetails row) {
        JSONObject jsonobj = MessageDetails.getAsJSONObject(row);
        if( jsonobj != null ){
            mMessageArray.put(jsonobj);
        }
        mMessageArray = JSONUtils.truncateJSONArray(mMessageArray, 10);  // truncate the oldest 10.
